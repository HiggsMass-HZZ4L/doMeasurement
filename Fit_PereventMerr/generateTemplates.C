...
const TString destDir = "./templates2D/";
...
pair<TH2F*,TH2F*> reweightForCRunc(TH2F* temp){
  ...
  for(int i=1; i<=temp->GetNbinsX(); i++){
    ...
    for(int p=0; p<numPoints; p++){
      ...
      float m=temp->GetXaxis()->GetBinCenter(i+1); 
      if( m>=low[p] && m<high[p] ){
	point = p;
      }
    }
...
void buildChain(TChain* bkgMC, TString channel, int sampleIndex=0) {
  ...
  if(sampleIndex==0){
    ...
    //qqZZ
//    bkgMC->Add("/scratch/osg/dsperka/Run2/HZZ4l/SubmitArea_13TeV/rootfiles_MC_Run2Fid_20160127/ZZTo4L_13TeV_powheg_pythia8_RunIIFall15MiniAODv2-PU25nsData2015v1_76X_mcRun2_asymptotic_v12-v1.root");
    bkgMC->Add("/scratch/osghpc/dsperka/Run2/HZZ4l/SubmitArea_13TeV/Ana_ZZ4L_dev/qqZZ.root");

  } else if (sampleIndex==2){
    //ggZZ

bkgMC->Add("/scratch/osghpc/dsperka/Run2/HZZ4l/SubmitArea_13TeV/Ana_ZZ4L_dev/qqZZ.root");
  //  bkgMC->Add("/scratch/osg/dsperka/Run2/HZZ4l/SubmitArea_13TeV/rootfiles_MC_Run2Fid_20160127/GluGluToContinToZZTo2e2mu_13TeV_MCFM701_pythia8_RunIIFall15MiniAODv2-PU25nsData2015v1_76X_mcRun2_asymptotic_v12-v1.root");
  //  bkgMC->Add("/scratch/osg/dsperka/Run2/HZZ4l/SubmitArea_13TeV/rootfiles_MC_Run2Fid_20160127/GluGluToContinToZZTo4e_13TeV_MCFM701_pythia8_RunIIFall15MiniAODv1-PU25nsData2015v1_76X_mcRun2_asymptotic_v12-v1.root");
  //  bkgMC->Add("/scratch/osg/dsperka/Run2/HZZ4l/SubmitArea_13TeV/rootfiles_MC_Run2Fid_20160127/GluGluToContinToZZTo4mu_13TeV_MCFM701_pythia8_RunIIFall15MiniAODv1-PU25nsData2015v1_76X_mcRun2_asymptotic_v12-v1.root");
...
TH2F* fillTemplate(TString channel="4mu", int sampleIndex=0,bool isLowMass=true){
  TChain* bkgMC = new TChain("passedEvents");
...
//  int fs;
  ...
  float pt4l=0, Y4l=0;
  float psig=0, pbkg=0;
  int idL1=0, idL2=0, idL3=0, idL4=0; 
  ...
  bkgMC->SetBranchAddress("mass4lErr",&KD);
//  double  KD_cut= 1.;      //       Set it to one, just to fix it to something meaningful (it's unused anyhow)
  bkgMC->SetBranchAddress("mass4l",&mzz);
  bkgMC->SetBranchAddress("mass4e",&m4e);
  bkgMC->SetBranchAddress("mass4mu",&m4mu);
  bkgMC->SetBranchAddress("mass2e2mu",&m2e2mu);
  bkgMC->SetBranchAddress("idL1",&idL1);
  bkgMC->SetBranchAddress("idL2",&idL2);
  bkgMC->SetBranchAddress("idL3",&idL3);
  bkgMC->SetBranchAddress("idL4",&idL4);


//  bkgMC->SetBranchAddress("finalState", &fs);
  
  //TH2F* bkgHist;
/*  if(1!isLowMass)
    bkgHist = new TH2F("bkgHisto","bkgHisto",int((highMzz-180.)/mBinSize+0.5),180,highMzz,30,0,1);
  else*/
  TH2F*  bkgHist = new TH2F("bkgHisto","bkgHisto",int((150-100)/mBinSize+0.5),100,150,100,0.0005, 0.1);//0.1,10.1);
  ...
  for(int i=0; i<bkgMC->GetEntries(); i++){
    ...
    bkgMC->GetEntry(i);
    if(mzz<0) continue;
    if ( !(mzz > 100 && mzz < 150)) continue;

       if ( channel == "4mu" && !(abs(idL1)==13 && abs(idL2)==13 && abs(idL3)==13 && abs(idL4)==13) ) continue;
       if ( channel == "4e" && !(abs(idL1)==11 && abs(idL2)==11 && abs(idL3)==11 && abs(idL4)==11) ) continue;
       if ( channel == "2e2mu" && !( (abs(idL1)==13 && abs(idL2)==13 && abs(idL3)==11 && abs(idL4)==11) || (abs(idL1)==11 && abs(idL2)==11 && abs(idL3)==13 && abs(idL4)==13)) ) continue;

//    if ( channel == "4mu" && m4mu <= 0) continue;
//    if ( channel == "4e" && m4e <= 0) continue;
//    if ( channel == "2e2mu" && m2e2mu <= 0) continue;
    ...
    if(i%100000==0) cout << "event: " << i << "/" << bkgMC->GetEntries() << endl;
      ...
      weight = 1.0;
      bkgHist->Fill(mzz,KD/mzz,weight);
  ...
  if(!isLowMass){
	...
	//	binMzz=(i-1)*2+181;
	float binMzz = bkgHist->GetXaxis()->GetBinCenter(i);
...
TH2F* mergeTemplates(TH2F* lowTemp, TH2F* highTemp){
  ...
  TH2F* h_mzzDm = new TH2F("h_mzzDm","h_mzzDm",int((highMzz-100.)/mBinSize +0.5),100,highMzz,nYbins,0,1);

  // copy lowmass into h_mzzDm
  for(int i=1; i<=lowTemp->GetNbinsX(); ++i){
    for(int j=1; j<=nYbins; ++j){
      h_mzzDm->SetBinContent(i,j, lowTemp->GetBinContent(i,j)  );
    }// end loop over D
  }// end loop over mZZ

  // copy high mass into h_mzzDm
  for(int i=1; i<=highTemp->GetNbinsX(); ++i){
    for(int j=1; j<=nYbins; ++j){
      h_mzzDm->SetBinContent(i+lowTemp->GetNbinsX(),j, highTemp->GetBinContent(i,j)  );
    }// end loop over D
  }// end loop over mZZ
...
void makeTemplate(TString channel="4mu"){
  ...
  TH2F* low,*high,*h_mzzDm;
  
  // ========================================
  // SM Higgs template
  
  TH2F* sigh_mzzDm = fillTemplate(channel,0,true);

  //high = fillTemplate(channel,0,false);
  //h_mzzDm = mergeTemplates(low,high);
  ...
  sigh_mzzDm->Write("h_mzzDm");
  //oldTemp->Write("oldTemp");
  sigh_mzzDm->Write("h_mzzDm_up");
  sigh_mzzDm->Write("h_mzzDm_dn");
  fsig->Close();
  ...
  h_mzzDm = fillTemplate(channel,1,true);
  //high = fillTemplate(channel,1,false);
  //h_mzzDm = mergeTemplates(low,high);

  // ---------- apply interference reweighting --------
  
  oldTemp = new TH2F(*h_mzzDm);
  oldTemp->SetName("oldTemp");

  //cout << "apply systematics for zjets control region" << endl;
  
  //histoPair = reweightForCRunc(h_mzzDm);

  // --------------------------------------------------

  fqqZZ->cd();
  h_mzzDm->Write("h_mzzDm");
  oldTemp->Write("oldTemp");
  h_mzzDm->Write("h_mzzDm_up");
  h_mzzDm->Write("h_mzzDm_dn");
  fqqZZ->Close();

  // ==========================
  // ggZZ templates
  
  TH2F* ggzzh_mzzDm = fillTemplate(channel,2,true);
  //high = fillTemplate(channel,2,false);
  //h_mzzDm = mergeTemplates(low,high);

  // ---------- apply interference reweighting --------
  
  oldTemp = new TH2F(*ggzzh_mzzDm);
  oldTemp->SetName("oldTemp");
  ...
  fggZZ->cd();
  ggzzh_mzzDm->Write("h_mzzDm");
  oldTemp->Write("oldTemp");
  ggzzh_mzzDm->Write("h_mzzDm_up");
  ggzzh_mzzDm->Write("h_mzzDm_dn");
  fggZZ->Close();
  ...
  //make as if ZZ
 TH2F* zxh_mzzDm = fillTemplate(channel,1,true);
  ...
  oldTemp = new TH2F(*zxh_mzzDm);
  oldTemp->SetName("oldTemp");

  //cout << "apply systematics for zjets control region" << endl;
  
//  histoPair = reweightForCRunc(zxh_mzzDm);

  // --------------------------------------------------
/*
  fZX->cd();
  //Write ZZ up template as ZX default
  histoPair.first->Write("h_mzzDm");
  oldTemp->Write("oldTemp");
  //Write ZX + (ZX-ZZ) as ZX up
  TH2F* ZXup = new TH2F(*h_mzzDm);
  for(int ZXbinx = 1; ZXbinx < ZXup->GetNbinsX(); ZXbinx++)
    {
      ...
      float diffTempValue = oldTempValue - (h_mzzDm->GetBinContent(ZXbinx,ZXbiny));
  ...
  ZXup->Write("h_mzzDm_up");
  //Write ZZ default tempalte as ZX down
  zxh_mzzDm->Write("h_mzzDm_dn");
  fZX->Close();
