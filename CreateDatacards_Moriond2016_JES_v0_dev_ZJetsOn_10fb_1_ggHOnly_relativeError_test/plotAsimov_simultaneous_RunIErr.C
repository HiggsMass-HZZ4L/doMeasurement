void plotAsimov_simultaneous_RunIErr(TString channel) {
    ...
    TFile::Open("higgsCombine1D_refit_Run-I-err.MultiDimFit.mH125.root");

    RooWorkspace *w = gFile->Get("w");
    RooAbsData * data = w->data("toy_asimov");

    cout<<"load snap shot"<<endl;
    w->loadSnapshot("clean");

    const RooSimultaneous *sim_orig  = dynamic_cast<const RooSimultaneous *>(w->pdf("model_s"));
    RooAbsPdf *pdfi_orig  = sim_orig->getPdf("hzz4l_13TeV_"+channel);

    RooProduct* ggh_orig = w->function("n_exp_final_binhzz4l_13TeV_"+channel+"_proc_ggH_hzz");
    RooProduct* qqh_orig = w->function("n_exp_final_binhzz4l_13TeV_"+channel+"_proc_qqH_hzz");
    RooProduct* wh_orig = w->function("n_exp_final_binhzz4l_13TeV_"+channel+"_proc_WH_hzz");
    RooProduct* zh_orig = w->function("n_exp_final_binhzz4l_13TeV_"+channel+"_proc_ZH_hzz");
    RooProduct* tth_orig = w->function("n_exp_final_binhzz4l_13TeV_"+channel+"_proc_ttH_hzz");

    RooProduct* ggzz_orig = w->function("n_exp_binhzz4l_13TeV_"+channel+"_proc_bkg_ggzz");
    RooProduct* qqzz_orig = w->function("n_exp_binhzz4l_13TeV_"+channel+"_proc_bkg_qqzz");
    RooProduct* zjets_orig = w->function("n_exp_binhzz4l_13TeV_"+channel+"_proc_bkg_zjets");
    ...
    c->SaveAs("fitresult_"+bin+"_simultaneous_1Drefit_Run-I-err.pdf");
    c->SaveAs("fitresult_"+bin+"_simultaneous_1Drefit_Run-I-err.png");
