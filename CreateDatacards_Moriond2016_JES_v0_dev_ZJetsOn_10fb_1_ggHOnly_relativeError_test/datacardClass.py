...
class datacardClass:
	...
	if dmHigh >= 1 :   # use absolute mass error as observable
        	...
        	if (self.channel == self.ID_4mu) :
        	    rfv_MassErr = ROOT.RooFormulaVar("rfv_MassErr_1","@1*@0*(1+@2)",ROOT.RooArgList(MassErr, self.MH, CMS_zz4l_sigma_m_sig))
        	elif (self.channel == self.ID_4e) :
        	    rfv_MassErr = ROOT.RooFormulaVar("rfv_MassErr_2","@1*@0*(1+@2)",ROOT.RooArgList(MassErr, self.MH, CMS_zz4l_sigma_e_sig))
        	elif (self.channel == self.ID_2e2mu) :
        	    rfv_MassErr = ROOT.RooFormulaVar("rfv_MassErr_3","@1*@0*TMath::Sqrt((1+@2)*(1+@3))",ROOT.RooArgList(MassErr, self.MH, CMS_zz4l_sigma_m_sig,CMS_zz4l_sigma_e_sig))
	...
	#----------------------- Begin  bIncludingError PDFs
        ...
        """
        name = "massErrS_ln_kappa_{0:.0f}".format(self.channel)
        rfv_EBE_sig_gs_sigma = ROOT.RooFormulaVar(name, "("+theInputs['relerr_ggH_gs_sigma']+")*@0", ROOT.RooArgList(self.MH))
        name = "massErrS_ln_mean_{0:.0f}".format(self.channel)
        rfv_EBE_sig_gs_mean = ROOT.RooFormulaVar(name, "("+theInputs['relerr_ggH_gs_mean']+")*@0", ROOT.RooArgList(self.MH))
        name = "errGaus_ggH_{0:.0f}".format(self.channel)
        EBE_sig_gs = ROOT.RooGaussian(name,name, MassErr, rfv_EBE_sig_gs_mean, rfv_EBE_sig_gs_sigma)
        name = "massErrS_ld_sigma_{0:.0f}".format(self.channel)
        rfv_EBE_sig_ld_sigma = ROOT.RooFormulaVar(name, "("+theInputs['relerr_ggH_ld_sigma']+")*@0", ROOT.RooArgList(self.MH))
        name = "massErrS_ld_mean_{0:.0f}".format(self.channel)
        rfv_EBE_sig_ld_mean = ROOT.RooFormulaVar(name, "("+theInputs['relerr_ggH_ld_mean']+")*@0", ROOT.RooArgList(self.MH))
        name = "errLD_ggH_{0:.0f}".format(self.channel)
        EBE_sig_ld = ROOT.RooLandau(name,name, MassErr, rfv_EBE_sig_ld_mean, rfv_EBE_sig_ld_sigma)
        name = "massErrS_ld_frac_{0:.0f}".format(self.channel)
        rfv_EBE_sig_frac = ROOT.RooFormulaVar(name, "("+theInputs['relerr_ggH_ld_frac']+")", ROOT.RooArgList(self.MH))
        name = "pdfErrS_{0:.0f}".format(self.channel)
        pdfErrS = ROOT.RooAddPdf(name,name, EBE_sig_ld, EBE_sig_gs, rfv_EBE_sig_frac)
	...
	bkg_zjetsErr = ROOT.RooProdPdf(name,name, ROOT.RooArgSet(bkg_zjets), ROOT.RooFit.Conditional(ROOT.RooArgSet(pdfErrZX), ROOT.RooArgSet(MassErr)));
      ...
      ## ----------------- 2D BACKGROUND SHAPES --------------- ##
        ...
        rdsprod = sig_ggHErr.generate(ROOT.RooArgSet(CMS_zz4l_mass,MassErr), 1)
...
#        bkg_qqzzErr.plotOn(zzerrframe_s, ROOT.RooFit.LineStyle(2), ROOT.RooFit.LineColor(2) )
        ...
        rates['qqH'] = smallNumber#sigRate_VBF_Shape
        rates['WH']  = smallNumber#sigRate_WH_Shape
        rates['ZH']  = smallNumber#sigRate_ZH_Shape
        rates['ttH'] = smallNumber#sigRate_ttH_Shape

        rates['qqZZ']  = smallNumber#bkgRate_qqzz_Shape
        rates['ggZZ']  = smallNumber#bkgRate_ggzz_Shape
        rates['zjets'] = smallNumber#bkgRate_zjets_Shape#  turn off Z+jets
        rates['ttbar'] = smallNumber
        rates['zbb']   = smallNumber
        
        for key in rates:
            rates[key] = rates[key]*10/2.8
